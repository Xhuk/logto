...
- `MachineToMachine` has no login page. `SPA` and `Traditional` still send people through the Experience SPA. Do not set redirect URIs with these tools; this MCP cannot patch them.
...
| `Native`, `SPA` | No client secret. Say that redirect URIs still need the Admin Console |

## Execution Steps

1. `logto_list_applications` for `tenant_id`. Reuse an app with the same name and type.
2. `logto_create_application` with `name` and `type`.
